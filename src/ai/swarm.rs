...
impl SwarmMessage {
    pub fn serialize(&self) -> Vec<u8> {
        let mut buffer = Vec::with_capacity(128); // Pre-allocate to reduce resizing
        match self {
...
pub async fn swarm_task() {
    crate::serial_println!("Swarm: Multi-agent orchestration engine online.");
    ...
    loop {
        ...
        let uptime_s = crate::interrupts::TICKS.load(Ordering::SeqCst) / 10;
        let cpu_load = 5;

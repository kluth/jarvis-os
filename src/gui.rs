...
const BG_PANEL: Color = Color { r: 12, g: 12, b: 20 };
const BG_PANEL2: Color = Color { r: 10, g: 10, b: 18 };
const BORDER_DIM: Color = Color { r: 18, g: 18, b: 30 };
const BORDER_GLOW: Color = Color { r: 0, g: 60, b: 90 };
const CYAN: Color = Color { r: 0, g: 220, b: 255 };
const CYAN_DIM: Color = Color { r: 0, g: 140, b: 200 };
const CYAN_GLOW: Color = Color { r: 0, g: 80, b: 140 };
const CYAN_BG: Color = Color { r: 0, g: 30, b: 50 };
const GREEN: Color = Color { r: 0, g: 255, b: 150 };
const WHITE: Color = Color { r: 200, g: 200, b: 200 };
const WHITE_DIM: Color = Color { r: 100, g: 100, b: 115 };
const PURPLE: Color = Color { r: 160, g: 60, b: 255 };
const YELLOW: Color = Color { r: 220, g: 200, b: 0 };
...
// DRAWING PRIMITIVES
// ============================================================================
fn draw_glow_rings(writer: &mut crate::vga_buffer::FramebufferWriter, cx: isize, cy: isize, max_r: isize, step: isize, fade_max: u8) {
    let mut r = max_r;
    while r > 0 {
        let t = r as f32 / max_r as f32;
        let fade = (fade_max as f32 * (1.0 - t)) as u8;
        let glow = Color { r: 0, g: fade / 2, b: fade };
        draw_circle_px(writer, cx, cy, r, glow);
        r -= step;
    }
}

fn draw_circle_px(writer: &mut crate::vga_buffer::FramebufferWriter, cx: isize, cy: isize, r: isize, color: Color) {
    let info = writer.get_info();
    let fb_w = info.width as isize;
    let fb_h = info.height as isize;
    let mut x: isize = 0; let mut y: isize = r; let mut d = 3 - 2 * r;
    while y >= x {
        for &(dx, dy) in &[(x, y), (y, x), (y, -x), (x, -y), (-x, -y), (-y, -x), (-y, x), (-x, y)] {
            let px = cx + dx; let py = cy + dy;
            if px >= 0 && px < fb_w && py >= 0 && py < fb_h { writer.write_pixel(px as usize, py as usize, color); }
        } x += 1; if d > 0 { y -= 1; d += 2 * (x - y) + 1; } else { d += 2 * x + 1; }
    }
}

fn draw_filled_circle(writer: &mut crate::vga_buffer::FramebufferWriter, cx: isize, cy: isize, r: isize, color: Color) {
    let info = writer.get_info();
    let fb_w = info.width as isize; let fb_h = info.height as isize;
    for dy in -r..=r {
        let half = libm::sqrtf(((r * r - dy * dy) as f32).max(0.0)) as isize;
        let x1 = (cx - half).max(0).min(fb_w - 1);
        let x2 = (cx + half).max(0).min(fb_w - 1);
        let py = cy + dy;
        if py >= 0 && py < fb_h { for px in x1..=x2 { writer.write_pixel(px as usize, py as usize, color); } }
    }
}

fn draw_arc(writer: &mut crate::vga_buffer::FramebufferWriter, cx: isize, cy: isize, r: isize, start_deg: f32, end_deg: f32, color: Color, thickness: isize) {
    let info = writer.get_info();
    let fb_w = info.width as isize; let fb_h = info.height as isize;
    for i in 0..=180 {
        let t = i as f32 / 180.0; let angle = start_deg + (end_deg - start_deg) * t;
...
// ICONS — 7×7 pixel art
// ============================================================================
fn draw_bitmap(writer: &mut crate::vga_buffer::FramebufferWriter, x: usize, y: usize, bmp: &[u8], color: Color) {
    for (dy, &row) in bmp.iter().enumerate() {
        for dx in 0..7 { if (row >> (6 - dx)) & 1 != 0 { writer.write_pixel(x + dx, y + dy, color); } }
    }
}

fn draw_icon(writer: &mut crate::vga_buffer::FramebufferWriter, x: usize, y: usize, t: usize, color: Color) {
    const ICONS: [&[u8]; 9] = [
        // 0: square
    ...
    if t < ICONS.len() { draw_bitmap(writer, x, y, ICONS[t], color); }
}
...
fn draw_static(writer: &mut crate::vga_buffer::FramebufferWriter) {
    ...
    // Full background
    writer.fill_rect(Rect { x: 0, y: 0, width: w, height: h }, BG_DARK);

    // === HEADER ===
    writer.fill_rect(Rect { x: 0, y: 0, width: w, height: HEADER_H }, BG_PANEL);
    writer.fill_rect(Rect { x: 0, y: HEADER_H - 1, width: w, height: 1 }, BORDER_GLOW);
    ...
    writer.fill_rect(Rect { x: pill_x as usize, y: 7, width: 180, height: 18 }, CYAN_BG);
    writer.draw_rect(Rect { x: pill_x as usize, y: 7, width: 180, height: 18 }, CYAN_GLOW);
    ...
    writer.fill_rect(Rect { x: SIDEBAR_X, y: SIDEBAR_TOP, width: SIDEBAR_W, height: 295 }, BG_PANEL);
    writer.draw_rect(Rect { x: SIDEBAR_X, y: SIDEBAR_TOP, width: SIDEBAR_W, height: 295 }, BORDER_DIM);
    ...
    writer.fill_rect(Rect { x: PANEL1_X, y: SIDEBAR_TOP, width: PANEL1_W, height: PANEL1_H }, BG_PANEL);
    writer.draw_rect(Rect { x: PANEL1_X, y: SIDEBAR_TOP, width: PANEL1_W, height: PANEL1_H }, BORDER_DIM);
    writer.write_string_at(PANEL1_X + 12, SIDEBAR_TOP + 8, "SUBSTRATE HEALTH", CYAN_DIM);

    // === SWARM VISUALIZER ===
    let sw_x = w - SWARM_W - 10;
    writer.fill_rect(Rect { x: sw_x, y: SWARM_TOP, width: SWARM_W, height: SWARM_H }, BG_PANEL);
    writer.draw_rect(Rect { x: sw_x, y: SWARM_TOP, width: SWARM_W, height: SWARM_H }, BORDER_DIM);
    ...
    writer.fill_rect(Rect { x: 10, y: log_y, width: w - 20, height: LOG_H }, BG_PANEL);
    writer.draw_rect(Rect { x: 10, y: log_y, width: w - 20, height: LOG_H }, BORDER_DIM);

    // === NAV DOCK ===
    let dock_y = h - DOCK_H - 6;
    let center_x = w / 2;
    writer.fill_rect(Rect { x: center_x - DOCK_W/2, y: dock_y, width: DOCK_W, height: DOCK_H }, BG_PANEL);
    writer.draw_rect(Rect { x: center_x - DOCK_W/2, y: dock_y, width: DOCK_W, height: DOCK_H }, BORDER_GLOW);
    ...
    for i in 0..5 {
        ...
        if i == 0 { writer.fill_rect(Rect { x: nx - 5, y: dock_y + 2, width: nav_item_w + 10, height: DOCK_H - 4 }, CYAN_BG); }
        let ix = nx + (nav_item_w - ICON_SIZE) / 2;
        draw_icon(writer, ix, dock_y + 4, icons_idx[i], colors[i]);
        writer.write_string_at(nx + (nav_item_w - labels[i].len() * 8) / 2, dock_y + DOCK_H - 10, labels[i], colors[i]);
    }
...
pub async fn ui_task() {
    ...
    loop {
        ...
        if let Some(writer) = WRITER.lock().as_mut() {
            ...
            for dy in -5..5 {
                for dx in -5..5 {
                    writer.write_pixel(px + dx as usize, py + dy as usize, 
                        Color { r: pulse as u8, g: pulse as u8, b: pulse as u8 });
                }
            }
        }
...
fn update(angle: f32, tick: u64) {
    if let Some(writer) = WRITER.lock().as_mut() {
        ...
        // === 1. CPU GAUGE ===
        let gc_x = PANEL1_X + 75; let gc_y = SIDEBAR_TOP + 78; let gr = 30;
        let cpu = 45 + ((tick.wrapping_mul(7) ^ (tick >> 2)) % 20) as u8;

        writer.fill_rect(Rect { x: PANEL1_X + 5, y: SIDEBAR_TOP + 30, width: PANEL1_W - 10, height: PANEL1_H - 38 }, BG_PANEL);
        ...
        let sa = -2.35619; let tot = 4.71239;
        draw_arc(writer, gc_x as isize, gc_y as isize, gr, sa, sa + tot, BORDER_DIM, 6);
        let ac = if cpu < 70 { GREEN } else if cpu < 90 { YELLOW } else { Color { r: 255, g: 50, b: 50 } };
        draw_arc(writer, gc_x as isize, gc_y as isize, gr, sa, sa + tot * fill, ac, 6);
        ...
        let bx = PANEL1_X + 120; let by = SIDEBAR_TOP + 80;
        writer.fill_rect(Rect { x: bx, y: by, width: 195, height: 8 }, BORDER_DIM);
        ...
        writer.fill_rect(Rect { x: (ccx - cr - 10) as usize, y: (ccy - cr - 10) as usize, width: cb as usize, height: cb as usize + 30 }, BG_DARK);
        ...
        // CORE_ACTIVE text
        let cl = "CORE_ACTIVE";
        writer.write_string_at((ccx - (cl.len() as isize * 4) - 10) as usize, (ccy + cr + 12) as usize, cl, CYAN);

        // Sine waveform
        let wy = (ccy + cr + 35) as usize;
        let wa = 6.0 + libm::sinf(angle * 0.5) * 3.0;
        let wf = 3.0 + libm::sinf(angle * 0.3) * 0.5;
        writer.fill_rect(Rect { x: (ccx - 120) as usize, y: wy - 10, width: 240, height: 20 }, BG_DARK);
        ...
        for x in 0..200 {
            ...
            if ws < h && ws > 0 {
                ...
                if ws + 1 < h { writer.write_pixel(wx, ws + 1, CYAN_DIM); }
                if ws > 0 { writer.write_pixel(wx, ws - 1, CYAN_DIM); }
            }
        }

        // === 3. SWARM VISUALIZER ===
        let sw_x = w - SWARM_W - 10;
        writer.fill_rect(Rect { x: sw_x + 3, y: SWARM_TOP + 20, width: SWARM_W - 6, height: SWARM_H - 23 }, BG_PANEL);
        ...
        for i in 0..9 {
            ...
            let nx = (hub_x as isize + (nd * libm::cosf(na)) as isize).max(sw_x as isize + 10).min(sw_x as isize + SWARM_W as isize - 10) as usize;
            let ny = (hub_y as isize + (nd * libm::sinf(na)) as isize).max(SWARM_TOP as isize + 25).min(SWARM_TOP as isize + SWARM_H as isize - 10) as usize;
            positions.push((nx, ny));

            writer.draw_line(hub_x as isize, hub_y as isize, nx as isize, ny as isize, Color { r: 0, g: 60, b: 100 });
            ...
            for j in 0..i {
                ...
                if dx < 200 && dy < 200 {
                    ...
                    if dist < 250.0 {
                        ...
                        writer.draw_line(nx as isize, ny as isize, ox as isize, oy as isize, Color { r: 0, g: fade / 3, b: fade / 2 });
                    }
                }
            }

            let nr = (3.0 + libm::sinf(angle + i as f32 * 2.0) * 1.5 + 2.0) as isize;
            draw_filled_circle(writer, nx as isize, ny as isize, nr + 2, Color { r: 0, g: 30, b: 50 });
            draw_filled_circle(writer, nx as isize, ny as isize, nr, CYAN);
        }

        let hr = (6.0 + libm::sinf(angle * 1.8) * 3.0) as isize;
        draw_filled_circle(writer, hub_x as isize, hub_y as isize, hr + 4, Color { r: 0, g: 40, b: 70 });
        ...
        // === 4. LOG WINDOW ===
        let log_y = h - LOG_H - DOCK_H - 12;
        writer.fill_rect(Rect { x: 13, y: log_y + 3, width: w - 26, height: LOG_H - 6 }, BG_PANEL2);

        let mi = (tick as usize / 15) % LOG_LINES.len();
        for li in 0..3 {
            let idx = (mi + li) % LOG_LINES.len();
            let msg = LOG_LINES[idx];
            let lc = if msg.starts_with("[NET]") || msg.starts_with("[SYS]") { CYAN }
                     else if msg.starts_with("[EVO]") { GREEN }
                     else if msg.starts_with("[SEC]") { YELLOW }
                     else { CYAN_DIM };
            writer.fill_rect(Rect { x: 16, y: log_y + 5 + li * LOG_GAP, width: w - 32, height: 12 }, BG_PANEL2);
